...
def test_window_average_2d():
    ...
    expected = np.array([[1.5, 2.0, 2.5], [4.5, 5.0, 5.5], [7.5, 8.0, 8.5]])  # Averaging along rows
    np.testing.assert_array_equal(result, expected)
...
def test_get_timed_clusters():
    ...
    result = get_timed_clusters(pool, cluster_labels, time_idx=3, fraction=0.5)
    expected = np.array([1])  # Cluster 1 is active at time_idx=3
    np.testing.assert_array_equal(result, expected)
...
def test_get_common_segments():
    ...
    expected = pd.DataFrame({
        0: [np.nan, 2, 3, 4, np.nan],
        1: [np.nan, 7, 8, 9, np.nan]
    })
    pd.testing.assert_frame_equal(result, expected)
'''