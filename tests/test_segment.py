...
def test_dynamic_unimodal_basic_case():
    ...
    inflections, directions = dynamic_unimodal(data, penalty=0.5)
    
    # Assert the inflections and directions are as expected
    assert list(inflections) == [0, 3, 5]
    assert list(directions) == [1, 0]
...
def test_dynamic_unimodal_single_element():
    data = np.array([1.0])
    inflections, directions = dynamic_unimodal(data, penalty=0.5, normalize=False)
    
    # Assert the segmentation handles single-element arrays correctly
    assert list(inflections) == [0, 1]
    assert list(directions) == [1] or list(directions) == [0]
...
def test_dynamic_unimodal_small_penalty():
    data = np.array([1.0, 2.0, 3.0, 2.0, 1.0])
    inflections, directions = dynamic_unimodal(data, penalty=0.01)
...
def test_dynamic_unimodal_no_penalty():
    data = np.array([1.0, 2.0, 3.0, 2.0, 1.0])
    inflections, directions = dynamic_unimodal(data, penalty=0.0)
    
    # With no penalty, the function should allow all segments
    assert list(inflections) == [0, 1, 2, 3, 4, 5]
    assert list(directions) == [1, 0, 1, 0, 1] or list(directions) == [0, 1, 0, 1, 0]
